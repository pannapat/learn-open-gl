...
const char *WINDOWTITLE = { "OpenGL / GLUT Sample -- Pannapat Helicopter Graphics" };
...
// Look at view
const bool OUTSIDE_VIEW = 0;
const bool INSIDE_VIEW = 1;
...
GLuint HeliCopterList;			// helicopter display list
GLuint TopBladeList;				// blade display list
GLuint RearBladeList;				// blade display list
bool ViewMode;					// look at position either INSIDE_VIEW or OUTSIDE_VIEW
float BladeAngle;
bool Frozen;
...
#define TOP_BLADE_WIDTH 1
#define TOP_BLADE_SPEED 5
const double TOP_BLADE_X = 0.;
const double TOP_BLADE_Y = 2.9;
const double TOP_BLADE_Z = -2.;

#define REAR_BLADE_RADIUS 1.5
#define REAR_BLADE_WIDTH 1
#define REAR_BLADE_SPEED 15
const double REAR_BLADE_X = .5;
const double REAR_BLADE_Y = 2.5;
const double REAR_BLADE_Z = 9.;

float Time;
#define MS_IN_THE_ANIMATION_CYCLE 10000; 	// HOW MANY MILLISECONDS ARE IN THE ANIMATION CYCLE
...
Display( )
{
	...
	{
		gluLookAt( -0.4, 1.8, -4.9,    0.,2,-10.,    0., 1., 0.  );
	}
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., (char *)"Pannapat's Helicopter" );
...
void DrawBlades()
{
	...
	glNewList(TopBladeList, GL_COMPILE);
		glPushMatrix( );
		glTranslatef( TOP_BLADE_X, TOP_BLADE_Y, TOP_BLADE_Z );
		glRotatef(BladeAngle * TOP_BLADE_SPEED,0,1,0);
		glRotatef(90,1,0,0);
		glBegin( GL_TRIANGLES );
			glColor3f( 1., 1., 1. );
	...
	glNewList(RearBladeList, GL_COMPILE);
		glPushMatrix( );
		glTranslatef( REAR_BLADE_X, REAR_BLADE_Y, REAR_BLADE_Z );
		glRotatef(BladeAngle * REAR_BLADE_SPEED, 1, 0, 0);
		// glRotatef( -90., 0., 0., 1. );
		glRotatef(90,0,1,0);
		glBegin( GL_TRIANGLES );
			glColor3f( 0., 0., 1. );
