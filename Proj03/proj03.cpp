...
const float GLOBE_RADIUS	= 7;
const int GLOBE_SLICES		= 500;
const int GLOBE_STACKS		= 500;
...
unsigned char *Texture;			// an array of unsigned characters holding the texel colors
bool	Distort;		// global -- true means to distort the texture
bool	TextureEnvReplace;		
bool	ShowTexture;		
float Time;
...
void	DoTextureEnvReplaceMenu(int);
void	DoShowTexture( int );
...
Display( )
{
	...
	gluLookAt( 0., 0., 10.,     0., 0., 0.,     0., 1., 0. );
		// gluLookAt( 12., 12., 12.,     0., 0., 0.,     0., 12., 0. );
...
void
DoTextureEnvReplaceMenu (int id )
{
	TextureEnvReplace = id;
	glutSetWindow( MainWindow );
	glutPostRedisplay();
}

void
DoShowTexture (int id)
{
	ShowTexture = id;
	glutSetWindow( MainWindow );
	glutPostRedisplay();
...
DrawSphere()
{
	...
	if(ShowTexture)
	{
		...
		Sphere(GLOBE_RADIUS, GLOBE_SLICES, GLOBE_STACKS);
	if(ShowTexture)
	{
...
InitMenus( )
{
	...
	int envreplacemenu = glutCreateMenu(DoTextureEnvReplaceMenu);
	glutAddMenuEntry("Modulate", 0);
	glutAddMenuEntry("Replace", 1);

	int showtexturemenu = glutCreateMenu(DoShowTexture);
	...
	glutAddSubMenu("Texture", showtexturemenu);
	glutAddSubMenu("Texture Env", envreplacemenu);
...
InitGraphics( )
{
	...
	Texture = BmpToTexture( "worldtex.bmp", &TEXTURE_WIDTH, &TEXTURE_HEIGHT );
	...
	for( int ilat = 0; ilat < NumLats; ilat++ )
	{
		...
		for( int ilng = 0; ilng < NumLngs; ilng++ )
		{
			...
			if( Distort )
			{
				// glMatrixMode( GL_TEXTURE );
				// TODO: TO BE CHANGED
				p->s = sin(Time*lat/M_PI) + ( lng + M_PI    ) / ( 2.*M_PI );
				p->t = cos(Time*lng/M_PI) + ( lat + M_PI/2. ) / M_PI;
			}
			else
			{
				p->s = ( lng + M_PI    ) / ( 2.*M_PI );
